...
-- 1. Dump into a file
\! psql -q -t -A -c 'SELECT anon.dump()' contrib_regression > tests/tmp/_dump1.sql
...
\! psql -f tests/tmp/_dump1.sql contrib_regression >/dev/null

-- 3. Dump again into a second file
\! psql -t -A -c 'SELECT anon.dump()' contrib_regression > tests/tmp/_dump2.sql

-- 4. Check that both dump files are identical
\! diff tests/tmp/_dump1.sql tests/tmp/_dump2.sql
