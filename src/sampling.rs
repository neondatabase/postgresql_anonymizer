...
pub fn get_ratio(relid: pg_sys::Oid, policy: &str)
-> Result<&str, masking::Reason>
{ get_table_ratio(relid, policy).or(get_current_database_ratio(policy)) }


fn get_current_database_ratio(policy: &str)
-> Result<&str, masking::Reason>
{
    let current_db_id = unsafe { pg_sys::MyDatabaseId };
    let seclabel = masking::rule_on_database(current_db_id,policy)?;
    let Some(ratio) = re::capture_tablesample(seclabel)
                      else { return Err(masking::Reason::InvalidInput); };
    Ok(ratio)
}

pub fn get_table_ratio(relid: pg_sys::Oid, policy: &str)
-> Result<&str, masking::Reason>
{
